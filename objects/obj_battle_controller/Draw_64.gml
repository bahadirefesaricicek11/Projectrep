        ...
        for (var _i = 0; _i < _card_count; _i++) { 
            ...
            if (sprite_exists(_base_sprite)) {
                ...
                if (sprite_exists(box_sprite)) { 
                    draw_sprite_ext(box_sprite, _frame_index, _center_x, _center_y, _x_scale * (_card_w / sprite_get_width(box_sprite)), _scale_multiplier * (_card_h / sprite_get_height(box_sprite)), _rot_angle, _box_color, _layer_alpha); 
                } else { 
...
if (global.state == GAME_STATE.BATTLE) { 
    ...
    for (var _p = 0; _p < _p_count; _p++) { 
        ...
        if (_m.hp > 0) { 
            ...
            var _final_draw_x   = _player_field_x + _input_bounce_x;

            if (sprite_exists(_m.sprite)) { 
                var _blend_color = _is_active_input ? c_white : c_silver; 
                draw_sprite_ext(_m.sprite, floor(_m.img_idx), _final_draw_x, _py, 1.0, 1.0, 0, _blend_color, 1.0); 
                ...
                if (_m.name == "Player" && variable_struct_exists(_m, "buffs") && is_array(_m.buffs)) {
                    for (var _b = 0; _b < array_length(_m.buffs); _b++) {
                        ...
                        if (is_struct(_buff_item) && variable_struct_exists(_buff_item, "sprite") && sprite_exists(_buff_item.sprite)) {
                            draw_sprite(_buff_item.sprite, 0, _final_draw_x - 16 + (_b * 12), _py - 24);
                        }
                    }
                }
            } 
    ...
    if (battle_sub_state == BATTLE_STATE.PLAYER_INPUT && menu_stage != BATTLE_MENU.MAIN && battle_text == "") { 
        ...
        switch (menu_stage) { 
            ...
            case BATTLE_MENU.HIT_BAR:
                ...
                var _side_padding = 16;
                var _track_w = _dash_w - (_side_padding * 2);
                var _track_h = _dash_h - 24; 
                var _track_x = _dash_x + _side_padding;
                var _track_y = _dash_y + (_dash_h - _track_h) / 2;
                
                draw_set_color(c_dkgray);
                draw_rectangle(_track_x, _track_y + (_track_h / 2) - 2, _track_x + _track_w, _track_y + (_track_h / 2) + 2, false);
                
                var _target_pixel_x = _track_x + (_track_w * hit_bar_target);
                draw_set_color(c_aqua);
                draw_rectangle(_target_pixel_x - 3, _track_y, _target_pixel_x + 3, _track_y + _track_h, false);
                
                var _reticle_pixel_x = _track_x + (_track_w * hit_bar_progress);
                if (hit_bar_progress > 0) {
                    draw_set_color(c_white);
                    draw_rectangle(_reticle_pixel_x - 5, _track_y - 2, _reticle_pixel_x + 5, _track_y + _track_h + 2, false);
                }
                
                if (hit_bar_verdict != "") {
                    draw_set_halign(fa_center);
                    var _verdict_color = (hit_bar_verdict == "PERFECT!") ? c_yellow : ((hit_bar_verdict == "GOOD") ? c_lime : c_red);
                    draw_text_transformed_color(_dash_x + (_dash_w / 2), _dash_y + 4, hit_bar_verdict, 0.5, 0.5, 0, _verdict_color, _verdict_color, _verdict_color, _verdict_color, 1.0);
                }
                break;
        } 
            ...
            if (_total_options == 0) {
                ...
                for (var _i = _start_index; _i < _end_index; _i++) {
                    ...
                    if (_is_inventory) {
                        ...
                        _display_text = (instance_exists(_element) && variable_instance_exists(_element, "name")) ? string(_element.name) : "Enemy";
                        if (instance_exists(_element) && variable_instance_exists(_element, "hp") && _element.hp <= 0) {

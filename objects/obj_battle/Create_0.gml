...
global.font_small = font_add_sprite(spr_font_small,ord("!"),true,1);
...
{
	enemyUnits[i] = instance_create_depth(x+290+(i*25), y+68+(i*20),depth-10, obj_battleUnitEnemy, enemies[i]);
	array_push(units, enemyUnits[i]);
}
for(var i = 0; i < array_length(global.party); i++)
{
	partyUnits[i] = instance_create_depth(x+50+(i*10), y+68+(i*15),depth-10, obj_battleUnitPC, global.party[i]);
...
function BattleStatePerformAction()
{
	if (currentUser.acting)
	{
		if(currentUser.image_index >= currentUser.image_number -1)
		{
			...
			if (variable_struct_exists(currentAction, "effectSprites"))
			{
